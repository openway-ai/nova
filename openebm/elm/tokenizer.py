"""Minimal tokenizer utilities for the EBT training stack.

Provides small tokenizer classes and a lightweight :class:`AutoTokenizer`
dispatcher that loads a tokenizer from a local directory by inspecting the
vocab files that are present. The implementations here cover only the subset
of tokenization used inside training and inference loops and are not intended
as full replacements for HuggingFace tokenizers.
"""
# NOTE: This file was originally generated by AI.
import os
import json
import re
...
def _read_vocab_txt(path: str) -> Dict[str, int]:
    ...
    :param path: Path to the ``vocab.txt`` file.
    :type path: str
    :return: Mapping from token string to integer id (line number).
    :rtype: Dict[str, int]
    """
    vocab: Dict[str, int] = {}
...
def _read_vocab_json(path: str) -> Dict[str, int]:
    """Read a ``vocab.json`` mapping token to id.

    :param path: Path to the ``vocab.json`` file.
    :type path: str
    :return: Mapping from token string to integer id.
    :rtype: Dict[str, int]
    :raises ValueError: If the file is not a JSON object or has non-int ids.
    """
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
...
def _whitespace_tokenize(text: str) -> List[str]:
    """Split ``text`` on whitespace after stripping.

    :param text: Input text.
    :type text: str
    :return: List of whitespace-separated tokens; empty if input is blank.
    :rtype: List[str]
    """
    text = text.strip()
    if not text:
...
def _basic_clean(text: str) -> str:
    """Apply a minimal subset of the cleaning done in real tokenizers.

    :param text: Input text.
    :type text: str
    :return: Text with runs of whitespace collapsed to single spaces and
...
def _pad_to_max(
    ...
    """Pad or truncate token ids to ``max_length`` and return attention mask.
    ...
    :type pad_id: int
    :return: Tuple ``(padded_ids, attention_mask)`` where the mask is 1 for
