...
### 2026-07-03 07:36 +0800

第五十四轮监控：

| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL local-log1 | `Running`，heartbeat 到 step 24 `generate_start` | `rl_events.jsonl` 到 step 23，共 24 条事件。last10 平均 reward `1.4017`、blank_accuracy `0.2693`、constraint_validity `0.1520`、full_solve `0.0250`，`LOCAL_ZERO=0/10`，max `ref_energy_kl=2.01e-4`。step 21 reward_mean `1.7412`、blank_accuracy `0.5027`；step 22/23 reward_mean `1.3332`/`1.1477`，KL 分别 `2.01e-4`/`1.27e-4`。 | 有效更新保持，local-zero 已连续 10 step 为 0；reward 波动但未塌缩。KL 峰值升到 `2e-4`，仍低于 `1e-3` 关注线；下一轮继续盯 KL 是否继续放大。 |
| GSM8K RL | `Running`，heartbeat 到 step 255 `skip_consensus_start` | local_skip `0`，unique_ratio `1.0`；控制面正常。 | 健康运行，继续保留。 |

### 2026-07-03 07:47 +0800
...
### 2026-07-03 09:49 +0800

第六十四轮监控：

| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL explore-anchor | `Running`，heartbeat 到 step 27 `generate_start` | `rl_events.jsonl` 到 step 26，共 27 条事件。last20 平均 reward `1.3648`、blank_accuracy `0.2904`、constraint_validity `0.1599`、full_solve `0.0500`，`LOCAL_ZERO=3/20`，max `ref_energy_kl=4.36e-5`。step 25 单次低质 local-zero，step 26 恢复有效更新：reward_mean `1.2198`、blank_accuracy `0.1653`、constraint_validity `0.1000`。 | 近期 reward 有下行波动，但不是连续 collapse；KL 稳定低位。继续观察 step 30，不重启。 |
| GSM8K RL | `Running`，heartbeat 到 step 306 `skip_consensus_start` | local_skip `0`，unique_ratio `1.0`；控制面正常。 | 健康运行，继续保留。 |

### 2026-07-03 10:01 +0800
...
### 2026-07-03 10:54 +0800

第六十八轮监控：

| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL diverse-slow | `Running`，heartbeat 到 step 7 `generate_start` | `rl_events.jsonl` 到 step 6，共 7 条事件。last5 平均 reward `1.2743`、reward_std `0.5044`、format `0.5000`、blank_accuracy `0.2041`、constraint_validity `0.1122`、full_solve `0.0150`，`LOCAL_ZERO=0/5`，max `ref_energy_kl=2.87e-7`。step 4/5/6 均为有效更新；step 6 reward_mean `1.6171`、blank_accuracy `0.4102`、constraint_validity `0.1862`、full_solve `0.0375`。 | diverse-slow 当前显著改善前一版后段的连续 local-zero：最近 5 step 没有零更新，reward 和 blank/validity 恢复，KL 仍接近 0。暂不重启，继续观察 step 10/20 是否维持。 |
...
### 2026-07-03 11:40 +0800

第七十二轮监控：

| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL diverse-slow | `Running`，heartbeat 到 step 17 `generate_start` | `rl_events.jsonl` 到 step 16，共 17 条事件。last5 平均 reward `1.4514`、reward_std `0.4382`、blank_accuracy `0.3160`、constraint_validity `0.1577`、full_solve `0.0300`，`LOCAL_ZERO=0/5`，max `ref_energy_kl=2.82e-5`。last10 平均 reward `1.4000`、full_solve `0.0637`，`LOCAL_ZERO=1/10`。step 14 reward_mean `1.9727`、full_solve `0.1500`；step 15 reward_mean `0.8562` 但 std `0.0574` 且未 skip；step 16 reward_mean `1.8042`、blank_accuracy `0.5021`、constraint_validity `0.3050`。 | 当前窗口健康：低 reward step 15 不是零方差/skip，后续 step 16 恢复高质量有效更新。KL 缓慢升高但仍远低于关注线，暂不重启。 |
| GSM8K RL | `Running`，heartbeat 到 step 349 `backward_done` | latest heartbeat：grad_norm `0.3306`、max_grad `0.3205`、nan_grad_params `0`；此前 step 340 reward_mean `0.2922`。 | GSM8K 仍健康推进，不重启。 |

本轮动作：仅记录监控结果，无代码改动、无 rjob 重启。下一轮继续观察 Sudoku step 20 前后的 last10，并关注 KL 是否继续抬升。

### 2026-07-03 11:51 +0800

第七十三轮监控：

| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL diverse-slow | `Running`，heartbeat 到 step 19 `skip_consensus_start`，local_skip `0`、unique_ratio `1.0` | `rl_events.jsonl` 到 step 18，共 19 条事件。last5 平均 reward `1.5124`、reward_std `0.3464`、blank_accuracy `0.3333`、constraint_validity `0.1913`、full_solve `0.0300`，`LOCAL_ZERO=0/5`，max `ref_energy_kl=5.27e-5`。last10 平均 reward `1.4072`、full_solve `0.0562`，`LOCAL_ZERO=1/10`。step 17/18 均为有效更新，reward_mean `1.7131`/`1.2157`，unique_completion_ratio 均为 `1.0`。 | 仍健康：step 11 后没有连续 local-zero，last10 reward 维持在 `1.4` 左右，KL 仍低。尚未到 step 20，继续观察。 |
| GSM8K RL | `Running`，heartbeat 到 step 352 `training_step_start` | step 345 reward_mean `0.2092`、reward_std `0.1515`、ref_energy_kl `0.0076`、grad_norm `0.5587`、nan_grad_params `0`；step 350 reward_mean `0.1445`、reward_std `0.0842`。 | GSM8K 仍健康推进，不重启。 |

本轮动作：仅记录监控结果，无代码改动、无 rjob 重启。下一轮继续看 Sudoku step 20 附近指标。

### 2026-07-03 12:02 +0800

第七十四轮监控：

| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL diverse-slow | `Running`，heartbeat 到 step 22 `skip_consensus_start`，local_skip `0`、unique_ratio `0.9375` | `rl_events.jsonl` 到 step 21，共 22 条事件。last5 平均 reward `1.4123`、reward_std `0.3146`、blank_accuracy `0.2703`、constraint_validity `0.1666`、full_solve `0.0075`，`LOCAL_ZERO=0/5`，max `ref_energy_kl=6.77e-5`。last10 平均 reward `1.4319`、reward_std `0.3764`、full_solve `0.0187`，`LOCAL_ZERO=0/10`。last20 平均 reward `1.3716`、`LOCAL_ZERO=1/20`。step 20/21 均为有效更新，reward_mean `1.7454`/`1.4752`。 | diverse-slow 到 step 21 仍稳定：最近 10 step 无 local-zero，明显优于 explore-anchor 后段的 `LOCAL_ZERO=4/10`。reward/full_solve 尚未收敛，但 rollout collapse 已显著缓解；KL 仍安全。 |
| GSM8K RL | `Running`，heartbeat 到 step 357 `training_step_start` | step 345/350/355 reward_mean `0.2092`/`0.1445`/`0.1248`，reward_std `0.1515`/`0.0842`/`0.1293`，parse_rate `1.0`/`0.875`/`0.750`，grad_norm step 350 `0.3739`，未见 NaN。 | GSM8K reward 有下行波动但仍有方差信号与正常梯度；暂不重启，继续监控。 |

本轮动作：仅记录监控结果，无代码改动、无 rjob 重启。下一轮继续观察 Sudoku step 30 前后是否维持低 local-zero，并关注 GSM8K reward 下行是否持续。

### 2026-07-03 12:14 +0800

第七十五轮监控：

| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL diverse-slow | `Running`，heartbeat 到 step 24 `skip_consensus_start`，local_skip `0`、unique_ratio `1.0` | `rl_events.jsonl` 到 step 23，共 24 条事件。last5 平均 reward `1.3242`、reward_std `0.4393`、blank_accuracy `0.2269`、constraint_validity `0.1210`、full_solve `0.0225`，`LOCAL_ZERO=0/5`，max `ref_energy_kl=6.77e-5`。last10 平均 reward `1.4183`、full_solve `0.0262`，`LOCAL_ZERO=0/10`；last20 `LOCAL_ZERO=1/20`。step 22/23 均为有效更新。 | Sudoku 仍稳定：最近 10 step 无 local-zero，rollout collapse 暂未复现。reward 稳定但 full_solve 仍低，尚不能判定收敛；继续观察 step 30。 |
...
### 2026-07-03 13:17 +0800

第八十一轮监控：

| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL diverse-slow | 控制面 `Running`，job `d26-ctx2048-sudoku-rl-fsdp2-merge-diverse-10-e5d45`；heartbeat 到 step 37 `skip_consensus_start`，local_skip `0` | `rl_events.jsonl` 到 step 36，共 37 条事件。last5 平均 reward `1.3384`、reward_std `0.3389`、full_solve `0.1275`，`LOCAL_ZERO=1/5`。last10 平均 reward `1.2398`、reward_std `0.3496`、full_solve `0.0675`，`LOCAL_ZERO=2/10`，max `ref_energy_kl=1.91e-4`。step 35/36 均为有效更新，reward_mean `0.8917`/`1.1796`，没有连续 local-zero。 | 从 13:04 的警戒状态恢复：last10 reward 回升，local-zero 频率下降，KL 仍低。继续当前 rjob，不重启。 |
...
### 2026-07-03 13:40 +0800

第八十三轮监控：

| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL diverse-slow | 控制面 `Running`，job `d26-ctx2048-sudoku-rl-fsdp2-merge-diverse-10-e5d45`；heartbeat 到 step 42 `skip_consensus_start`，local_skip `0` | `rl_events.jsonl` 到 step 41，共 42 条事件。last5 平均 reward `1.3385`、reward_std `0.5121`、full_solve `0.0375`，`LOCAL_ZERO=1/5`。last10 平均 reward `1.3385`、reward_std `0.4255`、full_solve `0.0825`，`LOCAL_ZERO=2/10`，max `ref_energy_kl=1.91e-4`。step 40/41 均为有效更新，reward_mean `1.7541`/`1.5516`，full_solve `0.1125`/`0.0375`。 | 从 13:28 的边缘状态恢复：last10 reward 回到 `1.34`，local-zero 频率下降，full_solve 保持非零。继续当前 rjob，不重启。 |
...
| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL diverse-slow | 控制面 `Running`，job `d26-ctx2048-sudoku-rl-fsdp2-merge-diverse-10-e5d45`；heartbeat 到 step 47 `generate_start` | `rl_events.jsonl` 到 step 46，共 47 条事件。last5 平均 reward `1.0388`、reward_std `0.3883`、format `0.4000`、blank_accuracy `0.1601`、constraint_validity `0.0989`、full_solve `0.0300`，`LOCAL_ZERO=1/5`。last10 平均 reward `1.1887`、reward_std `0.4502`、full_solve `0.0337`，`LOCAL_ZERO=2/10`，max `ref_energy_kl=9.95e-5`。step 45 为单次 local-zero，step 46 已恢复，reward_mean `1.3593`、std `0.4142`。 | 较 13:52 有回落，但不是连续 collapse：last10 仍高于 `1.0`，reward_std 非零，step 46 恢复有效更新。继续当前 rjob，不重启；下一轮重点看 step 47/48 是否继续恢复。 |
| GSM8K RL | 控制面 `Running`，job `d26-ctx2048-gsm8k-rl-fsdp2-merge-20260702-20-b18ca`；heartbeat 到 step 403 `skip_consensus_start`，local_skip `0` | step 400：reward_mean `0.2065`、reward_std `0.0918`、parse_rate `1.0`、ref_energy_kl `0.0137`、loss `0.00547`；step 395 backward grad_norm `0.3068`、nan_grad_params `0`。 | GSM8K 从 step 390 低点恢复，reward/std 与 parse 信号正常；不重启。 |

本轮动作：仅记录监控结果，无代码改动、无 rjob 重启。下一轮若 Sudoku last10 跌破 `1.0` 或 local-zero 连续化，再进入异常分析和可能调参。

### 2026-07-03 14:16 +0800

第八十六轮监控：

| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL diverse-slow | 控制面 `Running`，job `d26-ctx2048-sudoku-rl-fsdp2-merge-diverse-10-e5d45`；heartbeat 到 step 49 `skip_consensus_start`，local_skip `0` | `rl_events.jsonl` 到 step 48，共 49 条事件。last5 平均 reward `0.9851`、reward_std `0.3606`、format `0.4000`、blank_accuracy `0.1315`、constraint_validity `0.0805`、full_solve `0.0075`，`LOCAL_ZERO=1/5`。last10 平均 reward `1.2673`、reward_std `0.5173`、full_solve `0.0375`，`LOCAL_ZERO=1/10`，max `ref_energy_kl=0.00191`。step 47/48 均为有效更新，reward_mean `1.3068`/`1.2169`，step 48 full_solve `0.0375`。 | 相比 14:05，last10 明显恢复且 local-zero 降低；last5 低于 `1.0` 主要受 step 45 单次 local-zero 和低质量 step 44 影响。当前继续运行，不重启。 |
| GSM8K RL | 控制面 `Running`，job `d26-ctx2048-gsm8k-rl-fsdp2-merge-20260702-20-b18ca`；heartbeat 到 step 409 `training_step_start` | 最新完整指标仍为 step 400：reward_mean `0.2065`、reward_std `0.0918`、parse_rate `1.0`、ref_energy_kl `0.0137`、loss `0.00547`；未见新 Traceback 或 NaN/Inf。 | GSM8K 继续推进，暂无异常，不重启。 |

本轮动作：仅记录监控结果，无代码改动、无 rjob 重启。下一轮重点看 Sudoku last5 是否从 `0.9851` 回升，以及 KL 是否继续维持低量级。
...
| 任务 | 状态 | 指标快照 | 判断 |
| --- | --- | --- | --- |
| Sudoku RL diverse-slow | 控制面 `Running`，job `d26-ctx2048-sudoku-rl-fsdp2-merge-diverse-10-e5d45`；heartbeat 到 step 52 `generate_start` | `rl_events.jsonl` 到 step 51，共 52 条事件。last5 平均 reward `1.3189`、reward_std `0.3877`、format `0.5000`、blank_accuracy `0.2368`、constraint_validity `0.1106`、full_solve `0.0150`，`LOCAL_ZERO=0/5`。last10 平均 reward `1.1789`、reward_std `0.3880`、full_solve `0.0225`，`LOCAL_ZERO=1/10`，max `ref_energy_kl=0.00191`。step 51 reward_mean `2.0811`、blank_accuracy `0.7274`、constraint_validity `0.3181`、full_solve `0.0375`。 | last5 从 14:16 的 `0.9851` 明显恢复，最近 5 步没有 local-zero；KL 仍低。继续当前 rjob，不重启。 |
| GSM8K RL | 控制面 `Running`，job `d26-ctx2048-gsm8k-rl-fsdp2-merge-20260702-20-b18ca`；heartbeat 到 step 414 `training_step_start` | step 405：reward_mean `0.1376`、reward_std `0.1322`、parse_rate `1.0`、ref_energy_kl `0.0245`；step 410：reward_mean `0.2517`、reward_std `0.0807`、ref_energy_kl `0.0374`，未见 NaN/Inf。 | GSM8K 正常波动并在 step 410 回升，不重启。 |

本轮动作：仅记录恢复趋势，无代码改动、无 rjob 重启。下一轮继续关注 Sudoku 是否保持 `LOCAL_ZERO=0/5` 附近，以及 GSM8K step 410 backward 是否正常完成。
