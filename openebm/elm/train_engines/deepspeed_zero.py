...
def prepare_deepspeed_zero_args(args, engine: str) -> None:
    ...
    if stage == 3 and mcmc_gradient_mode not in {"first_order_cd", "first_order_nce", "proposal_aware_nce"}:
        raise NotImplementedError(
            "train_engine=zero-3 shards model parameters and is only enabled with "
            "--mcmc_gradient_mode first_order_cd/first_order_nce/proposal_aware_nce in this phase. "
    ...
    if stage == 3:
        _warn(
            ...
            "first_order_cd, first_order_nce, or proposal_aware_nce is recommended for lower sampler graph retention."
        )
