...
class EBTChatEngine:
    ...
    def generate(
        ...
        # 【修复点 2】: 增加 Chat Template 处理
        try:
            messages = [{"role": "user", "content": prompt}]
            prompt_tokens_list = self.tokenizer.apply_chat_template(messages, add_generation_prompt=True)
        except Exception:
            # Fallback 策略
            if hasattr(self.tokenizer, 'encode'):
                ...
                prompt_tokens_list = self.tokenizer(prompt)['input_ids']

            # 确保包含 BOS token
            bos_id = getattr(self.tokenizer, 'bos_token_id', None)
            if bos_id is not None and len(prompt_tokens_list) > 0 and prompt_tokens_list[0] != bos_id:
                prompt_tokens_list = [bos_id] + prompt_tokens_list

        # 【修复点 3】: 完全对齐 generate.py 中的 Tokenizer 特殊字符处理
        if hasattr(self.tokenizer, 'eos_token_id') and self.tokenizer.eos_token_id is not None:
            tokenizer_pad_token_id = self.tokenizer.eos_token_id
            ...
            tokenizer_pad_token_id = self.tokenizer.bos_token_id
        else:
            tokenizer_pad_token_id = 0  # Fallback to 0
        ...
        with torch.no_grad():
            ...
            for cur_pos in range(min_prompt_len, total_len):
                ...
                if stream and cur_pos >= min_prompt_len:
                    # 使用 skip_special_tokens 避免打印出内部标签
                    token_text = self.tokenizer.decode([next_token.item()], skip_special_tokens=True)
                    print(token_text, end='', flush=True)

                # EOS 检查（与 generate.py 100% 一致）
