...
class EBTChatEngine:
    ...
    def _load_model(self, checkpoint_path: str, tokenizer_path: str):
        ...
        # 与 eval 保持一致：只转移到 device，不改变 dtype
        # Checkpoint 中的权重已经是正确的 dtype（bfloat16）
    ...
    def _run_mcmc_with_tracking(
        ...
        与 generate.py 完全一致的调用方式
        """
        mcmc_history = []
        start_time = time.time()

        # 与 generate.py 完全一致：不使用 autocast，不传递 no_randomness
        with torch.no_grad():
            if getattr(self.hparams, 'infer_ebt_advanced', False):
                ebt_outputs = self.model.ebt_advanced_inference(input_ids, start_pos=0, learning=False)
                predicted_distributions = [ebt_outputs[0]]
                predicted_energies = [ebt_outputs[1]]
            else:
                ebt_outputs = self.model.forward(input_ids, start_pos=0, learning=False, return_raw_logits=True)
                predicted_distributions = ebt_outputs[0]
                predicted_energies = ebt_outputs[1]
        ...
        for step_idx in range(num_steps):
            logits = predicted_distributions[step_idx]
            energy = predicted_energies[step_idx]
    ...
    def generate(
        ...
        prompt_tokens_list = self.tokenizer.encode([prompt], prepend=bos_token, num_threads=1)[0]

        # 完全复制 generate.py 的逻辑
        tokenizer_pad_token_id = getattr(self.tokenizer, 'eos_token_id', getattr(self.tokenizer, 'bos_token_id', 0))

        bsz = 1
        prompt_tokens = [prompt_tokens_list]
        min_prompt_len = len(prompt_tokens_list)
        max_prompt_len = len(prompt_tokens_list)
        total_len = min(self.hparams.context_length, max_tokens + max_prompt_len)
        ...
        with torch.no_grad():
            # 特殊情况：prompt 已经填满整个 context（与 generate.py:159-166 一致）
            if min_prompt_len == total_len:
                if getattr(self.hparams, 'infer_ebt_advanced', False):
                    ebt_outputs = self.model.ebt_advanced_inference(tokens, start_pos=0, learning=False)
                    logits = ebt_outputs[0]
                else:
                    ebt_outputs = self.model.forward(tokens, start_pos=0, learning=False, return_raw_logits=True)
                    logits = ebt_outputs[0][-1]

            for cur_pos in range(min_prompt_len, total_len):
                input_tokens = tokens[:, :cur_pos]

                # 与 generate.py 完全一致的 forward 调用
                if getattr(self.hparams, 'infer_ebt_advanced', False):
                    ebt_outputs = self.model.ebt_advanced_inference(input_tokens, start_pos=0, learning=False)
                    logits = ebt_outputs[0]
                else:
                    ebt_outputs = self.model.forward(input_tokens, start_pos=0, learning=False, return_raw_logits=True)
                    logits = ebt_outputs[0][-1]
                ...
                # EOS 检查（与 generate.py 一致）
                eos_token_id = getattr(self.tokenizer, 'eos_token_id', getattr(self.tokenizer, 'bos_token_id', 0))
                eos_reached |= (~input_text_mask[:, cur_pos]) & (next_token == eos_token_id)
                prev_pos = cur_pos
                if all(eos_reached):
                    break

        # 提取生成的 tokens（与 generate.py 一致的后处理）
        out_tokens = []
        ...
        generated_text = self.tokenizer.decode(toks)
