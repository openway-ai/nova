...
################################################################################
# Submit d26 free-embedding direct-unembed Sudoku SFT on 2 nodes x 8 GPUs.
################################################################################

set -e

RUN_SCRIPT="/mnt/shared-storage-user/puyuan/code/OpenEBM/openebm/elm/runs/rjob/run_ebt_freeembed_sudoku_sft_2node_8gpu.sh"
...
rjob submit \
  --name=ebt-d26-freeembed-sudoku-sft-2n8g \
  --gpu=8 \
  --memory=1000000 \
  --cpu=100 \
