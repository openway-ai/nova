...
# VE (Value Embedding) 配置
# 启用后为交替层添加可学习的值嵌入，增强模型表达能力
# 设置为 true 启用，false 禁用 (默认禁用)
USE_VE=false
...
print_kv "Value Embedding (VE)" "$(if [ "$USE_VE" = "true" ]; then echo "${GREEN}启用${NC}"; else echo "禁用"; fi)"
...
$(if [ "$USE_VE" = "true" ]; then echo "--use_ve"; fi) \
\
