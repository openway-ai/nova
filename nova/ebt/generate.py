...
def get_ppl(model, batch, hparams, token_bytes=None): # computes teacher-forced metrics
    ...
    if token_bytes is not None:
        ...
        # (Adopted from origin: cleaner than the previous conditional define.)
        next_token_indices = full_ids[:, 1:].reshape(-1)
        if _need_chunked_loop:
            # Chunked loop builds `all_losses` left-to-right; trim to target
            # length if needed so bpb accounting stays aligned. We use
            # _need_chunked_loop (not just direct_block + size>1) so the
            # blockwise sequential_k1 path is also covered — that path also
            # goes through the chunked loop now.
