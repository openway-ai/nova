...
class Attention(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            ve (torch.Tensor, optional): Value embedding tensor for VE.
        ...
        # VE 注入：将 value embedding 加到 xv 上
...
class EBTDefault(nn.Module):
    ...
    def forward(
        ...
        Args:
            embeddings (torch.Tensor): Embeddings (instead of tokens since is for vision).
            start_pos (int): Starting position for attention caching.
            mcmc_step: MCMC step index (not used in default EBT).
            real_token_ids (torch.Tensor, optional): Real token IDs for VE.
            predicted_tokens (torch.Tensor, optional): Predicted token logits for VE.
