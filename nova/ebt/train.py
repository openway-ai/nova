...
def main(args):
    ...
    if args.only_test or args.only_validate:
        # Eval-time auto-override (from origin): silently align
        # training_objective to whatever block_mode the ckpt was trained
        # with, so eval callers only need to set --block_mode. Extended
        # to include future_latent_bidirectional (added by our branch).
        if args.block_mode in ("mtp_mcmc", "future_latent_non_causal", "blockwise", "future_latent_bidirectional"):
            if args.training_objective != "blockwise":
                print(
