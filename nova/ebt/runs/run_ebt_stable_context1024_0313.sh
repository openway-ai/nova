...
### 基础配置 ###
export RUN_NAME="ebt-d26-stable-context1024-6gpu"
...
#   4. --optimizer muon_adamw: Muon+AdamW 混合优化器 (复用 NanoChat 的 Muon)
#      Muon 处理 transformer 矩阵参数 (LR=0.02), AdamW 处理 embedding/scalar/alpha
#      预期收敛速度显著提升 (NanoChat 用 Muon 710步 loss<3, 纯 AdamW 67k步仍 3.52)
#
# 注意:
...
# OPTION_FLAGS="--optimizer muon_adamw --muon_lr 0.02 --muon_momentum 0.95 --muon_ns_steps 5 --muon_beta2 0.95"
