...
################################################################################
# EBT d26 blockwise 训练脚本 - Muon+AdamW 混合优化器 - 1节点 × 4卡
#
# 策略:
#   1. 模型: EBT d26
#   2. 训练目标: blockwise
...
#   5. 每节点申请卡数 < 8，必须使用 nccl_auto_config.py 自动配置 NCCL 参数
#
# 用法:
#   通过 rjob 提交 (见配套的 rjob_ebt_blockwise_1node_4gpu.sh)
#   本地调试:
#     NODE_RANK=0 NODE_COUNT=1 MASTER_ADDR=127.0.0.1 PROC_PER_NODE=4 \
#     bash run_ebt_blockwise_1node_4gpu.sh
################################################################################
...
### 基础配置 ###
RUN_PREFIX="1node-4gpu-bf16mixed-blockwise"
...
# Default to 1024 for the 1-node 4-GPU blockwise recipe; callers can still
