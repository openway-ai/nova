...
################################################################################
# EBT d26 blockwise 单机训练 rjob 提交脚本 - 1节点 × 4卡
#
# 用法:
#   bash rjob_ebt_blockwise_1node_4gpu.sh
#
# 可选环境变量:
...
rjob submit \
  --name=ebt-d26-blockwise-1node-4gpu \
  --gpu=4 \
  --memory=500000 \
  --cpu=40 \
  ...
  -- bash -exc "bash /mnt/shared-storage-user/lixueyan/nar/nova/nova/ebt/runs/rjob/run_ebt_blockwise_1node_4gpu.sh"
