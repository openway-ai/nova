...
class NanoChatEBT(nn.Module):
    ...
    (same has_ve pattern as GPT). forward() requires idx=[B, 2*(S-1)] token IDs;
    a zero time-placeholder is prepended internally to align with the sequence layout.
    """
    ...
    def __init__(self, params: EBTModelArgs, max_mcmc_steps: int):
        ...
        # window_pattern="L" → full causal context on all layers (no sliding window),
        # appropriate for energy-based models that need global context.
        ...
        # Window sizes for Flash Attention (all full context since pattern="L")
    ...
    def forward(self, embeddings: torch.Tensor, idx: torch.Tensor, start_pos: int, mcmc_step: int = 0):
        """
        Args:
        ...
        full_idx = F.pad(idx, (1, 0), value=0)   # [B, 2*(S-1)+1]
        ...
        for i, block in enumerate(self.blocks):
            ...
            ve = self.value_embeds[str(i)](full_idx) if str(i) in self.value_embeds else None
